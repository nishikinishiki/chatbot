...
function displayBannerImage(url) {
    ...
    let bannerElement;
    // ファイル拡張子が .mp4 の場合は video タグを作成
    if (url.toLowerCase().endsWith('.mp4')) {
        bannerElement = document.createElement('video');
        bannerElement.src = url;
        bannerElement.autoplay = true;   // 自動再生
        bannerElement.muted = true;      // ミュート（自動再生に必須）
        bannerElement.loop = true;       // ループ再生
        bannerElement.playsInline = true; // iOS等でのインライン再生用
    } else {
        // それ以外は従来通り img タグを作成
        bannerElement = document.createElement('img');
        bannerElement.src = url;
        bannerElement.alt = 'キャンペーンバナー';
    }

    bannerElement.className = 'chat-banner-image';

    bannerElement.onerror = () => {
        console.error('バナーの読み込みに失敗しました:', url);
