...
const GIFT_TERMS_DEFAULT =
  ...
  <ul>
    ...
    <li>お申込み後、お客様ご自身の希望により当社へ個人情報の削除を依頼・適用された場合</li>
  </ul>
...
const GIFT_TERMS_CAMPAIGN_HP =
  `<h3>えらべるデジタルギフト<br>プレゼント条件</h3>
  <h4>【個別面談・Web面談をお申込みのお客様】</h4>
  <p>プレゼントは、Web面談で20,000円、オフライン（対面）個別面談で50,000円相当のえらべるデジタルギフトを予定しております。<br>
  <br>
  【期間限定キャンペーン実施中！】<br>
  2026年9月1日(火) 00:00～2026年10月30日(金) 23:59の期間中にお申し込みいただいた方限定で、プレゼント金額を20,000円UPいたします。<br>
  Web面談：40,000円相当（通常 20,000円相当）<br>
  オフライン（対面）個別面談：70,000円相当（通常 50,000円相当）<br>
  <br>
  面談でえらべるデジタルギフトプレゼントは以下の条件を満たした方が対象となります。なお、Web面談、個別面談において当社のコンサルタントと当社オフィスもしくは当社オフィス外、ウェブ通信にて対面し、当社サービスの十分な説明とお客様についての十分な（数回にわたり）情報を相互に交換することが要件となります。<br>
  また、えらべるデジタルギフトではAmazonギフトカード、PayPayマネーライト、楽天ポイントからご希望の1種のみを選択いただけます。<br>
  ご希望のポイントは、フォームの「ご質問・ご要望」欄に記入ください。お申し込み時にご記入がない場合は、Amazonギフトカードをプレゼントいたします。お申し込み完了後の変更は原則受け付けておりませんので、予めご了承ください。<br>
  ...
  <br>
  【期間限定キャンペーンの対象条件】<br>
  ※20,000円UPの増額対象となるのは、2026年9月1日(火) 00:00～2026年10月30日(金) 23:59の期間中にフォームよりお申し込みを完了し、かつ以下の条件（お申し込みから90日以内の面談完了等）を満たした方となります。
  </p>
  <br>
  <ul>
    <li>①年収700万円以上の方（※主婦、パートの方は、配偶者様の年収が700万円以上の場合に対象となります）</li>
  ...
  const GIFT_TERMS_CONFIG = {
    ...
    get popup_content() {
      ...
      // 優先度が高い順に if 文で分岐して返す
      if (isCampaignHp) {
        return GIFT_TERMS_CAMPAIGN_HP;
      } else if (isCampaignAff) {
        return GIFT_TERMS_CAMPAIGN_AFF;
      } else {
        return GIFT_TERMS_DEFAULT; // どちらにも当てはまらない場合
      }
    }
  };
