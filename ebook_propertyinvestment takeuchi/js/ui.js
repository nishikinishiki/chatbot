...
function displayNormalInput(question, callbacks) {
    const wrapper = document.createElement('div');
    wrapper.className = 'input-container-wrapper';
    ...
    wrapper.innerHTML = `
        <div class="input-area" style="display: flex;">
          ...
          <button class="send-btn" disabled>${ICONS.ARROW_RIGHT}</button>
        </div>`;
    ...
    const btn = wrapper.querySelector('.send-btn');
    ...
    input.addEventListener('input', () => {
        const isValid = question.validation(input.value.trim());
    ...
    input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && !btn.disabled) {
            e.preventDefault(); // エンターキー本来の挙動（改行など）を防止
            handleSend();
        }
    });
}
...
function displayMultiChoices(question, onSelect) {
    const wrapper = document.createElement('div');
    wrapper.className = 'input-container-wrapper multi-choice-wrapper';
    ...
    submitBtn.className = 'multi-choice-submit-button';
...
function displayPairedInputs(question, onSubmit) {
    const wrapper = document.createElement('div');
    wrapper.className = 'input-container-wrapper paired-input-area-wrapper';
    const container = document.createElement('div');
    container.className = 'paired-input-container';
    
    const inputs = [];
    question.inputs.forEach((conf, idx) => {
        const row = document.createElement('div');
        row.className = 'paired-input-row';
        row.innerHTML = `<label>${conf.label}</label><input type="${conf.type || 'text'}" placeholder="${conf.placeholder || ''}">`;
        const input = row.querySelector('input');
        inputs.push(input);

        // 最後の入力欄（名、メイ）の場合
        if (idx === question.inputs.length - 1) {
            const btn = document.createElement('button');
            btn.className = 'paired-input-send-button';
            btn.innerHTML = ICONS.ARROW_RIGHT;
            btn.disabled = true;

            const handleFinalSubmit = () => {
                const vals = inputs.map(i => i.value.trim());
                if (question.combinedValidation(...vals)) {
                    onSubmit(vals, wrapper);
                } else {
                    addBotMessage(question.combinedErrorMessage, false, true);
                }
            };

            btn.onclick = handleFinalSubmit;

            input.oninput = () => {
                const ok = question.combinedValidation(...inputs.map(i => i.value.trim()));
                btn.disabled = !ok;
            ...
            spacer.className = 'paired-input-send-button placeholder';
...
function displayTimeTable(question, onSubmit) {
    ...
    const render = (startDate) => {
        ...
        subBtn.className = 'time-table-submit-button';
...
function showModal(title, content) {
    ...
    dom.giftTermsModal.style.display = 'flex';
}

function hideModal() {
    dom.giftTermsModal.style.display = 'none';
}
